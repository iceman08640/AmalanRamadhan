<x-ap-layout>
    <x-breadcrumb title="Imam dan Pembicara Management" page="Imam dan Pembicara Management" addButton="true">
        <a href="{{ route('kultum.create') }}"
    ...
    <div
        ...
        <div class="mb-4 flex flex-col sm:mb-0">
            <form class="flex flex-col items-start sm:flex-row sm:items-center" action="{{ route('kultum.index') }}"
                method="GET">
                    ...
                    <select name="tanggal" id="tanggal"
                        ...
                        @foreach ($list_agenda as $agenda)
                            <option value="{{ $agenda->id }}"
                                ...
                                {{ \Carbon\Carbon::parse($agenda->tgl)->translatedFormat('l, d M Y') .
                                    ($agenda->is_takbiran && $agenda->waktu
                                        ? ' - Takbiran, ' . \Carbon\Carbon::parse($agenda->waktu)->translatedFormat('H:i')
                                        : '') }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="flex w-full sm:w-auto">
                    ...
                    <a href="{{ route('kultum.index') }}" data-tooltip-target="tooltip-default"
                            ...
                            @forelse ($list_kultum as $kultum)
                                <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                    ...
                                    <td
                                        ...
                                        {{ \Carbon\Carbon::parse($kultum->agenda->tgl)->translatedFormat('l, d M Y') }}
                                    </td>
                                    <td
                                        class="whitespace-nowrap p-4 text-base font-medium text-gray-900 dark:text-white">
                                        {{ $kultum->imam->nama ?? '-' }}
                                    ...
                                    <td class="space-x-2 whitespace-nowrap p-4">
                                        <a href="{{ route('kultum.show', $kultum->id) }}"
                                        ...
                                        <button type="button" data-modal-toggle="delete-user-modal{{ $kultum->id }}"
                                ...
                                <div class="fixed left-0 right-0 top-4 z-50 hidden h-modal items-center justify-center overflow-y-auto overflow-x-hidden sm:h-full md:inset-0"
                                    id="delete-user-modal{{ $kultum->id }}">
                                        ...
                                        <div class="relative rounded-lg bg-white shadow dark:bg-gray-800">
                                            ...
                                            <div class="flex justify-end p-2">
                                                <button type="button"
                                                    ...
                                                    data-modal-toggle="delete-user-modal{{ $kultum->id }}">
                                            ...
                                            <div class="p-6 pt-0 text-center">
                                                ...
                                                <form action="{{ route('kultum.destroy', $kultum->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
